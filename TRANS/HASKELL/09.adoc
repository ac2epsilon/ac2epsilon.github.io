...
Цікава річ, з якою ми досі не зустрічались, це третій рядок, що вказує `name <- getLine`. Виглядає що вона читає рядок з входу, та зберігає її в змінній на ім'я `name`. Чи так насправді? Гаразд, давайте перевіримо тип `getLine`.
