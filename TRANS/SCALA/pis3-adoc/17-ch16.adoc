...
Lists are probably the most commonly used data structure in Scala programs. This chapter explains lists
...
16.1 LIST LITERALS
You saw lists already in the preceding chapters, so you know that a list containing the elements 'a', 'b',
and 'c' is written List('a', 'b', 'c'). Here are some other examples:
...
a linked list),footnote:[For a graphical depiction of the structure of a List, see Figure 22.2 here.] whereas arrays are flat.
16.2 THE LIST TYPE
...
natural because every list of strings can also be seen as a list of objects.footnote:[Chapter 19 gives more details on covariance and other kinds of variance.]
...
This can be expressed by giving append a type parameter,footnote:[Type parameters will be explained in more detail in Chapter 19.] that represents the element type of the two
input lists:
...
All that remains is to fill in the two places marked with ???.footnote:[The ??? method, which throws scala.NotImplementedError and has result type Nothing, can be used as a temporary implementation during development.] The first such place is the alternative
...
The splitAt operation splits the list at a given index, returning a pair of two lists.footnote:[As mentioned in Section 10.12, the term pair is an informal name for Tuple2.] It is defined by the
equality:
xs splitAt n
equals
(xs take n, xs drop n)
...
to a StringBuilder object,footnote:[This is class scala.StringBuilder, not java.lang.StringBuilder.] rather than returning them as a result:
...
higher-order operators,footnote:[By higher-order operators, we mean higher-order functions used in operator notation. As mentioned in Section 9.1, a function is "higher-order" if it takes one or more other functions as a parameters.] which are implemented as methods in class List. These higher-order
...
c, k), List(b, r, o, w, n), List(f, o, x))scala> words flatMap (_.toList)
...
parameters and explicit types. The second uses the concise form, (_ > _), where named parameters are
...
instance, in abcde.sortWith(_ > _), the type of abcde is List[Char]. Hence,sortWith is known to be a
...
written explicitly. With what it knows about sortWith, the inferencer can deduce that (_ > _) should
expand to ((x: Char, y: Char) => x > y) where x and y are some arbitrary fresh names.
Now consider the second case, msort(_ > _)(abcde). The type of msort is a curried, polymorphic
...
check the short-hand function literal, (_ > _), it fails because it has no information about the types of
...
used in turn to infer the type of the second parameter, (_ > _).
...
is List[Nothing]. Therefore, the operation (_ ::: _) of the fold is expected to be of the following type:
