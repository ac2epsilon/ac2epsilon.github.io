...
By now you have probably grown accustomed to the concise way data can be decomposed and
...
string str is an email address with the given user and domainparts,footnote:[As demonstrated here, where Some is applied to the tuple, (user, domain), you can leave off one pair of parentheses when passing a tuple to a function that takes a single argument. Thus,Some(user, domain) means the same as Some((user, domain)).] or None, if str is not an email
...
are used heavily in Scala's libraries, for instance, to enable convenient regular expression matching.
