...
This chapter introduces Scala's support for XML. After discussing semi-structured data in general, it
...
language, though, as it admits free-form text between tags and it lacks a type system.footnote:[There are type systems for XML, such as XML Schemas, but they are beyond the scope of this book.]
...
28.2 XML OVERVIEW
XML is built out of two basic elements, text and tags.footnote:[The full story is more complicated, but this is enough to be effective with XML.] Text is, as usual, any sequence of characters.
...
the \ operator:
scala>
<a><b><c>hello</c></b></a> ~"c"res11: scala.xml.NodeSeq = NodeSeq()
...
instead of a single one. The pattern for "any sequence" of XML nodes is written `_*'.Visually, this
sequence looks like the wildcard pattern (_) followed by a regex-style Kleene star (*). Here is the
...
a <cctherm> element. You can describe this subset using the pattern <cctherm>{_*}</cctherm>, and
