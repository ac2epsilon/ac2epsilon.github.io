...
instance of Int, 'x' is an instance of Char, and false an instance ofBoolean. You cannot create instances
...
instantiated
new Int
^The other value class, Unit, corresponds roughly to Java's void type; it is used as the result type of a
...
from AnyRef.footnote:[The reason AnyRef alias exists, instead of just using the name java.lang.Object, is because Scala was originally designed to work on both the Java and .NET platforms. On .NET, AnyRefwas an alias for System.Object.] One way to think of java.lang.Object, therefore, is as the way AnyRef is implemented
...
classes and compare their instances with reference equality.footnote:[You hash cons instances of a class by caching all instances you have created in a weak collection. Then, any time you want a new instance of the class, you first check the cache. If the cache already has an element equal to the one you are about to create, you can reuse the existing instance. As a result of this arrangement, any two instances that are equal withequals() are also equal with reference equality.] For these cases, class AnyRef defines an
...
composition. In the next chapter, you'll learn about traits.
