...
Кейс класи та співпадіння шаблонів
==================================

This chapter introduces case classes and pattern matching, twin constructs that support you when
...
expression being considered.footnote:[Instead of an abstract class, we could have also chosen to model the root of that class hierarchy as a
trait. Modeling it as an abstract class may be slightly more efficient.] The bodies of all five classes are empty. As mentioned previously, in
...
is bound within the associated pattern. The wildcard pattern (_) also matches every value, but it does
...
Here's an example:
UnOp("-", UnOp("-", e))
Imagine trying to implement this same functionality using the visitor design pattern!footnote:[Gamma, et. al., Design Patterns [Gam95\]]Almost as
...
value `()', which is also the result of the entire matchexpression.
...
to treat a keyword as an ordinary identifier, e.g., writingThread.`yield`() treats yield as an identifier
...
parentheses: "+", e, and Number(0). Assuming the name designates a case class, such a pattern means
...
The second case of the match expression in Listing 15.11 contains the typed pattern "m: Map[_, _]".
...
pattern, footnote:[In the typed pattern, m: Map[_, _], the "Map[_, _]" portion is called a type pattern.] are like wildcards in other patterns. You could have also used (lowercase) type variables
...
result of formatting the argument arg with the highest-possible environment precedence.footnote:[The value of unaryPrecedence is the highest possible precedence, because it was initialized to one more than the precedence of the * and % operators.] This means
