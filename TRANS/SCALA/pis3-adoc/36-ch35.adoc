...
In the previous chapters you saw many different constructs of the Scala programming language. In this
...
call this(row, column).footnote:[Although "this(row, column)" may look similar to a constructor invocation, it is in this case an invocation of the apply method on the current Table instance.] The userData method also takes care to display a null element as the empty
...
class Model(val height: Int, val width: Int)
extends Evaluator with Arithmetic {
Compared to the previous version of Model, this version adds a new import of swing._, which makes
...
getters, valueand formula, and setters, value_= and formula_=. Here are the definitions implementing
the valueproperty:
...
extends ScrollPane {
val cellModel = new Model(height, width)
import cellModel._val table = new Table(height, width) {
...
affected. One way to change this would be to add a redraw command to the value_= setter. However,
