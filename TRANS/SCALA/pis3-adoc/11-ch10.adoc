...
convention supports the uniform access principle,footnote:[Meyer, Object-Oriented Software Construction [Mey00\]] which says that client code should not be affected
...
method contents in class Element.footnote:[One flaw with this design is that because the returned array is mutable, clients could change it. For the book we'll keep things simple, but were ArrayElement part of a real project, you might consider returning a defensive copy of the array instead. Another problem is we aren't currently ensuring that every String element of the contents array has the same length. This could be solved by checking the precondition in the primary constructor and throwing an exception if it is violated.] By contrast, class ArrayElement inherits
...
class Element, and as a result, the type ArrayElement is compatible with the type Element.footnote:[For more perspective on the difference between subclass and subtype, see the glossary entry for subtype.]
...
parameterless method with a val, something you can't do with Java.footnote:[The reason that packages share the same namespace as fields and methods in Scala is to enable you to import packages (in addition to just the names of types) and the fields and methods of singleton objects. This is also something you can't do in Java. It will be described in Section 13.3.]
...
reassignable. Finally, it is possible to add modifiers, such as private, protected,footnote:[The protected modifier, which grants access to subclasses, will be covered in detail inChapter 13.] or override to these
...
fragile base class problem, but it improves on the situation compared to Java.footnote:[In Java 1.5, an @Override annotation was introduced that works similarly to Scala's overridemodifier, but unlike Scala's override, is not required.] If the drawing library
...
case, Element objects can have many forms.footnote:[This kind of polymorphism is called subtyping polymorphism. Another kind of polymorphism in Scala called universal polymorphism is discussed in Chapter 19.]
...
Element = ae
Element = new UniformElement('x', 2, 3)
...
a relationship.footnote:[Meyers, Effective C++ [Mey91\]] For example, it would be reasonable to say that ArrayElement is-an Element. Another
question you can ask is whether clients will want to use the subclass type as a superclass type.footnote:[Eckel, Thinking in Java [Eck98\]] In the
...
strings from its own contents field.footnote:[Class ArrayElement also has a composition relationship with Array, because its parametriccontents field holds a reference to an array of strings. The code for ArrayElement is shown inListing 10.5 here. Its composition relationship is represented in class diagrams by a diamond, as shown, for example, in Figure 10.1 here.] Given this implementation ofLineElement, the inheritance
...
The ++ operation concatenates two arrays. Arrays in Scala are represented as Java arrays, but support
...
Scala. We'll work with the layout library again inChapter 14.
