...
def filesEnding(query: String) =
filesMatching(query, _.endsWith(_))def filesContaining(query: String) =
...
syntax: _.endsWith(_). The first underscore is a placeholder for the first parameter, the file name, and
...
literals used in the previous example, such as _.endsWith(_) and _.contains(_), are instantiated at
runtime into function values that are not closures because they don't capture any free variables. Both
variables used in the expression, _.endsWith(_), for example, are represented by underscores, which
means they are taken from arguments to the function. Thus, _.endsWith(_) uses two bound variables,
and no free variables. By contrast, the function literal _.endsWith(query), used in the most recent
...
of Scala's collection types.footnote:[These special-purpose looping methods are defined in trait Traversable, which is extended by List, Set, and Map. See Chapter 17 for a discussion.] Many of these are listed inTable 3.1 in Chapter 3, but take a look at just
...
The underscore in curriedSum(1)_ is a placeholder for the second parameter list.footnote:[In the previous chapter, when the placeholder notation was used on traditional methods, like println _, you had to leave a space between the name and the underscore. In this case you don't, because whereas println_ is a legal identifier in Scala, curriedSum(1)_ is not.] The result is a
...
footnote:[You'll call this myAssert, not assert, because Scala provides an assert of its own, which will be described in Section 14.1.] The myAssert function will take a function value as input and consult a flag to decide what to do. If
...
few chapters will go back to discussing more object-oriented features of the language.
