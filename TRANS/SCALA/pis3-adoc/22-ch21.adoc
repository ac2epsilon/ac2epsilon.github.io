...
There's a fundamental difference between your own code and other people's libraries: You can change
...
As written so far, this code doesn't work.footnote:[As will be explained in Section 31.5, it does work in Scala 2.12.] The addActionListener method wants an action listener but
...
object definition. Here's an example of an implicit function definition:footnote:[Variables and singleton objects marked implicit can be used as implicit parameters. This use case will be described later in this chapter.]
...
useful implicit conversions. Code that uses the library can then do a single "import Preamble._" to
...
type system for this; it's just an implicit conversion that gets applied.footnote:[The Scala compiler backend will treat the conversion specially, however, translating it to a special "i2d" bytecode. So the compiled image is the same as in Java.]
...
bobsPrompt: PreferredPrompt = PreferredPrompt@714d36d6scala> Greeter.greet("Bob")(bobsPrompt)
...
scala> import JoesPrefs._
import JoesPrefs._Because both prompt and drink are now in scope as single identifiers, you can use them to supply the
