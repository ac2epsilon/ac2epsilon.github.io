...
Less (що розшифровується як Leaner Style Sheets) - це  зворотнє-сумісне розширення мови для CSS. Це офіційна документація для Less, мови та Less.js, інструменту JavaScript, який перетворює ваші Less стилі в стилі CSS.

Оскільки Less схоже на CSS, навчитися просто. Less вносить лише кілька зручних доповнень до мови CSS, що є однією з причин того, що його можна так швидко засвоїти.

* Докладнішу документацію про можливості мови Less див. Можливості
* Перелік вбудованих функцій Less див. Функції
* Детальні інструкції щодо використання див. Використання Less.js
* Про сторонні інструменти для Less див. Інструменти

Що додає Less до CSS? Ось короткий огляд можливостей.
...
Міксини - це спосіб включення ("змішування") купи властивостей з одного набору правил в інший набір правил. Тож скажімо, у нас такий клас:
...
І ми хочемо використовувати ці властивості всередині інших наборів правил. Гаразд, нам просто потрібно вписати ім'я класу, де ми хочемо властивості, як-от так:
...
Властивості класу `.borported` тепер з’являться в `#menu a` і `.post a`. (Зверніть увагу, що ви також можете використовувати `#ids` як міксин.)

=== Вкладання

Less дає вам можливість використовувати вкладання замість або в поєднанні з каскадуванням. Скажімо, у нас є такий CSS:
...
У Less ми можемо також записати це так:
...
Отриманий код є більш стислим і імітує структуру вашого HTML.

За допомогою цього методу ви також можете поєднати псевдоселектори зі своїми міксінами. Ось класичний `clearfix` хак, переписаний у вигляді міксіну (& являє собою поточний батьківський вибір):
...
=== Вкладені At-правила та спливання

At-правила, як `@media` або `@supports`, можуть вкладатись так само, як і селектори. Правило `at` розміщується зверху, а відносний порядок щодо інших елементів всередині того ж набору правил залишається незмінним. Це називається спливанням бульбашок.
...
Арифметичні операції +, -, *, / можуть оперувати будь-яким числом, кольором або змінною. Якщо можливо, математичні операції враховують одиниці та перетворюють числа перед їх додаванням, відніманням або порівнянням. Результат залишає самий лівий явно вказаний тип одиниць. Якщо конверсія неможлива або не має сенсу, одиниці ігноруються. Приклад неможливого перетворення: `px` в `cm` або `rad` у `%`.

[source,css]
----
// числа перетворюються в ті самі одиниці
@conversion-1: 5cm + 10mm; // результат 6cm
@conversion-2: 2 - 3cm - 5mm; // результат -1.5cm

// конверсія неможлива
@incompatible-units: 2 + 5px - 3cm; // результат 4px
...
Множення і ділення не перетворюють числа. Це не мало б сенсу у більшості випадків - довжина, помножена на довжину, дає площу, але css не підтримує завдання областей. Less буде працювати над числами, якими вони є, і присвоювати результату чітко вказаний тип одиниці.
...
Однак ви можете вважати кориснішими функції кольору Less.

=== calc() exception
Released v3.0.0

Для сумісності з CSS `calc()` не оцінює математичні вирази, але оцінить змінні та математику у вкладених функціях.
...
Виключення дозволяє використовувати будь-який довільний рядок як властивість або змінне значення. Все, що знаходиться всередині ~"anything" або ~"anything", використовується як є, без змін, крім інтерполяції.
...
Зауважте, що з Less 3.5 ви можете просто написати:
...
У 3.5+ більшість випадків, які раніше вимагали "лапок виключення", не потрібні.

=== Функції

Less надає різні функції, які перетворюють кольори, маніпулюють рядками та математикою. Вони повністю задокументовані у довіднику про функції.

Їх використання досить прямолінійне. Наступний приклад використовує `percentage` для перетворення 0,5 на 50%, збільшує насиченість основного кольору на 5%, а потім встановлює колір фону на той, який освітлюється на 25% і крутиться на 8 градусів:
...
=== Простори імен та аксессори

(Не плутати з CSS `@namespace` або селесторами `namespace`).

Іноді, можливо, ви захочете згрупувати свої міксини, в цілях впорядкування,або просто запропонувати деяку інкапсуляцію. Ви можете зробити це досить інтуїтивно в Less. Скажімо, що ви хочете з'єднати деякі міксини та змінні під `#bundle`, щоб пізніше повторно використовувати або розповсюдити:
...
Тепер, якщо ми хочемо поєднати клас `.button` у нашому `#header a`, ми можемо зробити:

[source,css]
----
#header a {
  color: orange;
  #bundle.button();  // або можна написати #bundle > .button
}
----

Примітка: додайте `()` до вашого простору імен (наприклад, `#bundle()`), якщо ви не хочете, щоб він відображався у вашому CSS-виході, тобто `#bundle .tab`.

=== Мапи

Станом на менше 3,5, ви також можете використовувати міксини та набори правил як мапи значень.
...
Поглиблений посібник щодо особливостей мови LESS.
